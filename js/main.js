...
var a = 1 + b;
console.log(a);
console.log(locationAxis.axisY);
...
var createAllCards = function () {
  for (var i = 0; i < 8; i++) {
    ...
    var createCardObject = {
      authorObjects : {
        avatarKey: 'img/avatars/user0' + (i + 1) + '.png'
      },
      location: {
        xObjectKey : getRandomInteger(X_MIN, X_MAX),
        yObjectKey : getRandomInteger(Y_MIN, Y_MAX)
      },
      offerObject: {
        titleObjectKey : 'Заголовок предложения',
        // На данный момент решения о передаче данных из location не нашел.
        addressObjectKey : "600, 350",
        ...
        priceObjectKey : PRICE_ARRAY[getRandomArrayIndex(PRICE_ARRAY)],
        typeObjectKey : TYPE_ROOM_ARRAY[getRandomArrayIndex(TYPE_ROOM_ARRAY)],
        roomsObjectKey : NUMBER_ROOMS_ARRAY[getRandomArrayIndex(NUMBER_ROOMS_ARRAY)],
        guestsObjectKey : NUMBER_GUESTS_ARRAY[getRandomArrayIndex(NUMBER_GUESTS_ARRAY)],
        checkinObjectKey : CHECKIN_ARRAY[getRandomArrayIndex(CHECKIN_ARRAY)],
        checkoutObjectKey : CHECKOUT_ARRAY[getRandomArrayIndex(CHECKOUT_ARRAY)],
        ...
        featuresObjectKey : FEATURES_ARRAY.slice(0, getRandomArrayIndex(FEATURES_ARRAY)),
        descriptionObjectKey : 'строка с описанием',
        photosObjectKey : photoArray.slice(0, getRandomArrayIndex(photoArray)),
      }
    }
    createCardArray.push(createCardObject);
    // var xCopy = createCardObject.location.xObjectKey;
